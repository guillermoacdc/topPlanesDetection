...
%     Criteria – Filter duplicates of ground. 
    if(abs(Dg)-abs(D)>minH/2)%definir el treshold igual a la altura mínima de caja en escena
        ...
        normals = pcnormals(pcRef);
        eval('ind_model=load([in_path num2str(scene) ''/index'' num2str(i) ''.txt'']);');
        ind_model=ind_model+1;%index in c++ begin at 0
        ptTemp=extractPoint(pcRef,ind_model);
        figure,
        pcshow(ptCloud{i+1},'MarkerSize', 40)
        hold on
        pcshow(ptTemp,'MarkerSize', 20)
        
        xn = double(pcRef.Location(ind_model,1));
        yn = double(pcRef.Location(ind_model,2));
        zn = double(pcRef.Location(ind_model,3));
        un = normals(ind_model,1);
        vn = normals(ind_model,2);
        wn = normals(ind_model,3);
