...
% carga modelo de plano
        ...
        xp=[min(x) max(x) ; min(x) max(x)];
        yp=[min(y) min(y); max(y) max(y) ];
        zp = -1/C*(A*xp + B*yp + D); % Solve for z data
% proyecta puntos en plano
        testout=myProjectPointsInPlane(x,y,z,A,B,C,D)
        return;
% calcula area de plano
